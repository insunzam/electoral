...
p <- X2021_11_CR_DE_DV %>% 
  filter(NumRegion == 9 & Comuna %in% c("TEMUCO")) %>% 
...
g21 <- ggplot(p1, aes(fill=Sexo, y=value, x=Comuna)) +
  ...
  ggtitle("Porcentaje de Votantes por Sexo 
          Temuco 2021 - Consejeros Regionales")
g23 <- ggplot(pc1, aes(fill=Sexo, y=V2023, x=Comuna)) +
  ...
  geom_text(aes(label = V2023), position = position_stack(vjust = 0.5),
            vjust = 0, color = "white", size=4) +
  ggtitle("Porcentaje de Votantes por Sexo 
          Temuco 2023 - Plebiscito Constitucional")
...
g + geom_text(aes(label = value), position = position_dodge(0.9),
  ...
  ggtitle("Total Comparado de Votantes Temuco por Sexo - 2021 - 2023")
...
g + geom_text(aes(label = value), 
              ...
              vjust = 0) +
  ggtitle("Total Comparado de Votantes Temuco por Rango de Edad - 2021 - 2023")

write.csv(pls, file = "~/R/electoral/data/votacion_sexo_local_Temuco_2021.csv")
write.csv(plre, file = "~/R/electoral/data/votacion_Retario_local_Temuco_2021.csv")
write.csv(pls, file = "~/R/electoral/data/votacion_sexo_local_Temuco_2023.csv")
write.csv(pls, file = "~/R/electoral/data/votacion_Retario_local_Temuco_2023.csv")
